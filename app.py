# app.py (궁극의 최종 버전 v2.6 Ultimate)
...
VIDEO_FORMATS = {
    ...
    "mkv": "MKV (고품질, 다중트랙 지원)",
    "webm": "WebM (웹 최적화, 고효율)",
    "mov": "MOV (Apple, 영상 편집용)",
    "avi": "AVI (구형, 범용적)",
    "flv": "FLV (플래시, 구형 웹)"
}
AUDIO_FORMATS = {
    "mp3": "MP3 (가장 일반적인 형식)",
    "m4a": "M4A (AAC 코덱, 좋은 음질)",
    "flac": "FLAC (무손실 음원, 원음 그대로)",
    "wav": "WAV (무압축 원음, 용량 큼)",
    "opus": "Opus (고효율, 스트리밍용)",
    "aac": "AAC (MP3보다 발전된 형식)"
}
AUDIO_QUALITY_MAP = {
...
if st.session_state.video_info:
    ...
    if download_type == "영상 + 음성":
        ...
        with col_q:
            available_res = get_available_resolutions(info)
            selected_res = st.selectbox("화질", available_res, key="quality_select")
        with col_f:
            selected_fps = st.selectbox("프레임 (강제 변환)", FRAME_RATES, key="fps_select")
        with col_e:
            selected_ext = st.selectbox("파일 형식", list(VIDEO_FORMATS.keys()), format_func=lambda x: VIDEO_FORMATS[x], key="video_ext")
    ...
    # --- 음원만 옵션 ---
        ...
        with col_a:
            selected_ext = st.selectbox("음원 형식", list(AUDIO_FORMATS.keys()), format_func=lambda x: AUDIO_FORMATS[x], key="audio_ext")
        with col_b:
    ...
    if st.button("다운로드 시작", use_container_width=True):
        ...
        output_path = f"{clean_title}.{selected_ext}"
        ...
        def progress_hook(d):
            if d['status'] == 'downloading':
                ...
                if total:
                    ...
                    progress_bar.progress(percent, text=f"다운로드 중... {int(percent * 100)}%")
            elif d['status'] == 'finished':
                 progress_bar.progress(1.0, text="파일 처리 및 변환 중...")

        ydl_opts = {'progress_hooks': [progress_hook], 'outtmpl': output_path, 'ffmpeg_location': '/usr/bin/ffmpeg', 'postprocessors': []}
        ...
        if download_type == "영상 + 음성":
            ...
            # 프레임 강제 변환 필터 추가
            ydl_opts['postprocessors'].append({'key': 'FFmpegVideoFilter', 'filters': f'fps={selected_fps}'})
            # 선택한 음질로 오디오를 재인코딩 하도록 설정
            ydl_opts['postprocessors'].append({'key': 'FFmpegExtractAudio', 'preferredcodec': 'aac', 'preferredquality': audio_quality_selector})
        ...
        try:
            ...
            with open(output_path, "rb") as file: file_bytes = file.read()
            st.session_state.download_result = { "file_name": output_path, "file_bytes": file_bytes }
